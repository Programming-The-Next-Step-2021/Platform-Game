...
class Fighter(pygame.sprite.Sprite):
    ...
    def move(self, moving_left: bool, moving_right: bool,
        ...
        for tile in obstacle_list:
            ...
            if tile[1].colliderect(self.rect.x, self.rect.y + dy, self.width,
                ...
                if self.speed_y < 0:  # if the speed is negative you are going up
                    ...
                    dy = tile[
                             1].bottom - self.rect.top  # change of position will be bottom of the tile - head (top)
                    # of character
                # check if above ground, whether he's falling
                    ...
                    dy = tile[1].top - self.rect.bottom  # if the change of position will be top of the tile - feet (
                    # bottom) character
                    self.jump = False  # prevents you from loading a jump while your in air which will be activated
                    # when you reach te ground
        ...
        if self.char_type == 'player':  # only if character is player
            if self.rect.left + dx < 0 or self.rect.right + dx > SCREEN_WIDTH:  # if you fall of left or right side
                # of lvl
        ...
        if self.char_type == 'player':  # only if character is player
            # if you are about to hit border of the screen: right of rect is > screen width - 200 pixels or left side
            # is smaller than the treshold (cause x coordinate star with 0)
    ...
    def update(self, player: Fighter) -> None:
    ...
    def ai(self, obstacle_list: list[tuple[pygame.Surface, pygame.Rect]]) -> None:
        ...
        if not self.idling:  # if they are not idling -> move
    ...
    def update_animation(self) -> None:
        ...
        if self.action == 3:  # speed up the animation for attack cause it has more images thus will be animated slower
        ...
        if self.frame_index >= len(self.animation_list[self.action]):  # if current image index is bigger than length
            # of all images of that action
            if self.attack:
                self.attack = False
